...
def get_items_from_sheet(username, target_status):
    ...
    # Оформление в стиле "Minimalist Shop"
    reply = f"<b>hellopinky / {target_status.upper()}</b>\n"
    reply += f"──────────────────\n"
...
def start(message):
    msg = bot.send_message(message.chat.id, "✨ <b>hellopinky</b>\n\nВведите ваш логин (с @):", parse_mode="HTML")
...
def show_menu(chat_id, user):
    text = f"👤 <b>Личный кабинет:</b> <code>{user}</code>"
...
def handle_query(call):
    ...
    if not user: return bot.send_message(uid, "Введите /start")
    ...
    if call.data == "track":
        ...
        bot.send_message(uid, res if res else "Позиций не найдено 🥺", parse_mode="HTML")
        ...
        msg = bot.send_message(uid, "📝 <b>Что оплачиваем?</b>", parse_mode="HTML")
        bot.register_next_step_handler(msg, pay_amt)

def pay_amt(message):
    item = message.text
    msg = bot.send_message(message.chat.id, f"💰 <b>Сумма для «{item}»:</b>", parse_mode="HTML")
    bot.register_next_step_handler(msg, pay_email, item)

def pay_email(message, item):
    try:
        amt = float(message.text.replace(' ', ''))
        msg = bot.send_message(message.chat.id, "📧 <b>Email для чека:</b>", parse_mode="HTML")
...
def pay_final(message, item, amt):
    ...
    if "@" not in email: return bot.send_message(message.chat.id, "❌ Неверный Email.")
